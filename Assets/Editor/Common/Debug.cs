        ...
        public static void MessageBox( String s ) {
            ...
            if ( showCallStackEnabled ) {
                using ( var prcShell = new System.Diagnostics.Process() ) {
                    ...
                    Common.UDebug.Log( prcShell.StartInfo.FileName );
        ...
        static public void Assert( bool condition, string assertString = null ) {
            if ( !condition ) {
                ...
                try {
                    ...
                    assertString = assertString ?? String.Empty;
                    string assertInformation = "Filename: " + myFrame.GetFileName() + "\nMethod: " + myFrame.GetMethod() + "\nLine: " + myFrame.GetFileLineNumber();
                    ULogFile.sharedInstance.LogError( assertInformation );
                    var s = assertString + "\n" + assertInformation;
                    LogError( s );
                    if ( _IgnoreCount < 5 ) {
                        UnityEngine.Debug.Break();
                        if ( UnityEditor.EditorUtility.DisplayDialog( "Assert!", s, "Debug", "Ignore" ) ) {
